...
YDB Materializer — это Java-приложение, которое обрабатывает управляемые пользователем материализованные представления в YDB.

[Скачать приложение можно на странице релизов](https://github.com/zinal/ydb-materializer/releases).

## Использование

Каждое «материализованное представление» (MV) технически представляет собой обычную таблицу YDB, которая обновляется с помощью этого приложения. Исходная информация извлекается из набора исходных таблиц, связанных друг с другом с помощью JOIN в стиле SQL. Для поддержки онлайн-синхронизации изменений исходных таблиц в MV используются потоки YDB Change Data Capture.

Таблицы назначения для MV, исходные таблицы, необходимые индексы и CDC-потоки должны быть созданы до использования приложения для синхронизации. Приложение может помочь сгенерировать части DDL для некоторых объектов — например, оно сообщает о недостающих индексах и может сгенерировать предлагаемую структуру таблиц назначения.

## Синтаксис языка материализованных представлений

YDB Materializer использует специальный SQL-подобный язык для определения материализованных представлений и обработчиков их обработки. Этот язык основан на подмножестве SQL с особыми расширениями для материализованных представлений YDB.
...
Предложение WHERE поддерживает непрозрачные YQL-выражения:
```sql
...
- **PROCESS**: указывает, какое материализованное представление обновляет этот обработчик.
...
- **CONSUMER**: необязательное имя потребителя для changefeed.
...
- Java 21 или выше.
- Кластер YDB с соответствующими разрешениями.
