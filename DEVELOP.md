...
- чтение ключей из основной таблицы (ReadTable API);
- заливка в MV через UPSERT SELECT;
- фиксация позиции обработки.

Основной поток задания читает ключи. Вспомогательный пул потоков заливает. Вспомогательный поток фиксирует позиции обработки. Между потоками - очереди (перед пулом - очередь с лимитом на размер).
